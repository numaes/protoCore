...
// Public-API variant of the cache race: every iteration writes and reads an
// attribute on a fresh object, which fills and clears attribute-cache
// entries (results are cells or SmallIntegers), while the 4096-cell budget
// runs a collection cycle every few thousand cells.  A small live set pinned
// in a root set must read back intact afterwards.  Before the thread caches
// became stop-the-world roots, this workload crashed the concurrent mark in
// about half of 30-second runs.
TEST(ConcurrentMarkSafety, AttributeCacheChurnWithSmallBudget) {
    ScopedEnv budget("PROTOCORE_GC_MIN_BUDGET_CELLS", "4096");
    ProtoSpace space;
    ProtoContext* root = space.rootContext;
    const ProtoString* key = sym(root, "churn");
    const ProtoString* tag = sym(root, "tag");

    constexpr int kLive = 64;
    ProtoRootSet* rs = space.createRootSet("attribute-cache-churn");
    std::vector<ProtoRootSet::Handle> handles;
    for (int i = 0; i < kLive; ++i) {
        ProtoObject* obj = const_cast<ProtoObject*>(root->newObject(true));
        obj->setAttribute(root, tag, root->newList()->appendLast(root, root->fromInteger(i))->asObject(root));
        handles.push_back(rs->add(obj));
    }

    const uint64_t cyclesStart = space.getGCCycleCount();
    unsigned long badReads = 0;
    const auto end = std::chrono::steady_clock::now() + std::chrono::seconds(3);
    while (std::chrono::steady_clock::now() < end) {
        ProtoContext sub(&space, root, nullptr, nullptr, nullptr, nullptr);
        const ProtoObject* list = sub.newList()->appendLast(&sub, sub.fromInteger(7))->asObject(&sub);
        for (int j = 0; j < 4096; ++j) {
            const ProtoObject* value = (j & 1) ? list : sub.fromInteger(j);
            const ProtoObject* obj = sub.newObject(false)->setAttribute(&sub, key, value);
            if (obj->getAttribute(&sub, key) != value) ++badReads;
            if ((j & 0x3F) == 0) sub.safepoint();
        }
    }
    const uint64_t cycles = space.getGCCycleCount() - cyclesStart;

    EXPECT_EQ(badReads, 0u);
    EXPECT_GE(cycles, 10u) << "too few collection cycles ran for the test to exercise the race";
    for (int i = 0; i < kLive; ++i) {
        const ProtoObject* obj = rs->resolve(handles[i]);
        ASSERT_NE(obj, nullptr);
        const ProtoObject* t = obj->getAttribute(root, tag);
        ASSERT_NE(t, nullptr);
        const ProtoList* l = t->asList(root);
        ASSERT_NE(l, nullptr);
