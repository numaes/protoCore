...
// ---------------------------------------------------------------------------
// Many threads: the budget is charged for cells consumed, not handed out.
// ---------------------------------------------------------------------------

namespace {

std::atomic<int> g_idleStarted{0};
std::atomic<bool> g_idleRelease{false};

// Allocate a few objects (the first allocation takes a refill batch), then
// idle inside an unmanaged region until released.
const ProtoObject* allocateThenIdle(ProtoContext* ctx, const ProtoObject*, const ParentLink*,
                                    const ProtoList*, const ProtoSparseList*) {
    for (int i = 0; i < 8; ++i) (void) ctx->newObject(false);
    g_idleStarted.fetch_add(1);
    ProtoContext::UnmanagedScope unmanaged(ctx);
    while (!g_idleRelease.load()) std::this_thread::sleep_for(std::chrono::milliseconds(1));
    return PROTO_NONE;
}

std::atomic<int> g_garbageObjects{0};

// Allocate g_garbageObjects objects of garbage in short-lived contexts, with
// a safepoint every 64 allocations, then return.
//
// After each context the thread waits, polling safepoints, while a cycle is
// pending or running.  Eight threads allocating flat out produce garbage
// faster than the single collector thread reclaims it, and the heap then
// grows during every cycle whatever the pacing; waiting keeps these tests
// about when cycles start, which is what the pacing decides.
const ProtoObject* allocateGarbage(ProtoContext* ctx, const ProtoObject*, const ParentLink*,
                                   const ProtoList*, const ProtoSparseList*) {
    const int total = g_garbageObjects.load();
    for (int done = 0; done < total;) {
        ProtoContext sub(ctx->space, ctx, nullptr, nullptr, nullptr, nullptr);
        for (int j = 0; j < 20000 && done < total; ++j, ++done) {
            (void) sub.newObject(false);
            if ((j & 0x3F) == 0) sub.safepoint();
        }
        while (ctx->space->gcStarted.load()) {
            ctx->safepoint();
...
bool globalFreelistEmpty(ProtoSpace& space) {
    std::lock_guard<std::recursive_mutex> lock(ProtoSpace::globalMutex);
    return space.freeChunks == nullptr && space.freeCells == nullptr;
}

}  // namespace

// Before the fix, every refill batch was charged to the allocation budget
// when a thread received it.  A thread that allocated a single cell was
// charged a whole batch (65,536 cells once several threads run), so starting
// threads that then went idle spent the budget without producing garbage and
// started a cycle.  Batches are now charged as they are consumed.
TEST(GCHeapGrowthTriggerTest, IdleThreadBatchesDoNotSpendTheBudget) {
    ScopedEnv growth("PROTOCORE_GC_GROWTH_PERCENT", nullptr);
    ScopedEnv budget("PROTOCORE_GC_MIN_BUDGET_CELLS", "600000");
    ProtoSpace space;
    ProtoContext* root = space.rootContext;
    constexpr int kThreads = 8;

    g_idleStarted.store(0);
    g_idleRelease.store(false);
    const uint64_t cyclesStart = space.getGCCycleCount();

    // A live context holds every cell the main thread allocates here, and no
    // safepoint runs, so nothing is submitted yet: no cycle can start.
    ProtoContext keeper(&space, root, nullptr, nullptr, nullptr, nullptr);

    // Start the threads; each takes one refill batch and goes idle.  Before
    // they start, drain the recycled cells published at bootstrap, so the
    // threads' refills come from the OS in full multi-thread batches.
    std::vector<const ProtoThread*> threads;
    {
        const ProtoThread* first = space.newThread(root, nullptr, allocateThenIdle, nullptr, nullptr);
        threads.push_back(first);
        while (!globalFreelistEmpty(space)) {
            for (int i = 0; i < 512; ++i) (void) keeper.newObject(false);
        }
        for (int t = 1; t < kThreads; ++t)
            threads.push_back(space.newThread(root, nullptr, allocateThenIdle, nullptr, nullptr));
    }
    while (g_idleStarted.load() < kThreads) std::this_thread::yield();

    // Submit a little garbage so the budget check is no longer deferred, then
    // allocate 70,000 cells, forcing at least one refill on the main thread.
    {
    ...
    // Give a requested cycle the chance to stop the world and run.
    for (int i = 0; i < 100; ++i) {
        keeper.safepoint();
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
    const bool requested = space.gcStarted.load();
    const uint64_t cycles = space.getGCCycleCount() - cyclesStart;

    g_idleRelease.store(true);
    for (const ProtoThread* t : threads) joinUnmanaged(&keeper, t);

    EXPECT_FALSE(requested) << "a cycle is pending although the threads consumed a few cells each";
    EXPECT_EQ(cycles, 0u) << "idle threads' unused batches spent the allocation budget";
}

// Charging on consumption must keep the heap bounded when many threads
// allocate garbage.
TEST(GCHeapGrowthTriggerTest, ManyThreadGarbageStillBoundsHeap) {
    ScopedEnv growth("PROTOCORE_GC_GROWTH_PERCENT", nullptr);
    ScopedEnv budget("PROTOCORE_GC_MIN_BUDGET_CELLS", "262144");
    ProtoSpace space;
    ProtoContext* root = space.rootContext;
    constexpr int kThreads = 8;
    constexpr int kObjectsPerThread = 500000;          // 1,000,000 cells each
    constexpr long kTotalCells = 2L * kThreads * kObjectsPerThread;

    g_garbageObjects.store(kObjectsPerThread);
    const int heapBefore = space.heapSize;
    const uint64_t cyclesStart = space.getGCCycleCount();
    std::vector<const ProtoThread*> threads;
    for (int t = 0; t < kThreads; ++t)
        threads.push_back(space.newThread(root, nullptr, allocateGarbage, nullptr, nullptr));
    for (const ProtoThread* t : threads) joinUnmanaged(root, t);

    const uint64_t cycles = space.getGCCycleCount() - cyclesStart;
    const long grown = static_cast<long>(space.heapSize) - heapBefore;
    EXPECT_GE(cycles, 3u) << "too few cycles for " << kTotalCells << " cells of garbage";
    EXPECT_LT(grown, kTotalCells / 4)
        << "heap grew by " << grown << " cells for " << kTotalCells << " cells of garbage";
}

// Threads that allocate less than one batch and exit never refill, so only
// exit settlement charges what they consumed.  Without it these threads
// would never start a cycle.  Their unused cells are also returned, so the
// heap stays bounded.
TEST(GCHeapGrowthTriggerTest, ShortLivedThreadsAreCharged) {
    ScopedEnv growth("PROTOCORE_GC_GROWTH_PERCENT", nullptr);
    ScopedEnv budget("PROTOCORE_GC_MIN_BUDGET_CELLS", nullptr);
    ProtoSpace space;
    ProtoContext* root = space.rootContext;
    ...
    constexpr int kObjectsPerThread = 15000;          // 30,000 cells each
    constexpr long kTotalCells = 2L * kRounds * kThreadsPerRound * kObjectsPerThread;
    ...
    // A cycle requested by the last exits may still be pending.
    for (int i = 0; i < 1000 && space.getGCCycleCount() == cyclesStart; ++i) {
        root->safepoint();
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
    ...
    const uint64_t cycles = space.getGCCycleCount() - cyclesStart;
    const long grown = static_cast<long>(space.heapSize) - heapBefore;
    EXPECT_GE(cycles, 2u) << kTotalCells << " cells consumed by short-lived threads started too few cycles";
    EXPECT_LT(grown, kTotalCells / 2)
        << "heap grew by " << grown << " cells for " << kTotalCells << " cells of garbage";
}
