// ProtoMapIteratorTests.cpp — ascending key-word iteration over a
// ProtoMap version, both in Small form (promoted internally)
// and AVL form.  D1 = (a) (PROTOMAP-SPEC §7): the iterator handle is an
...
namespace {
    ...
    PairVec drain(ProtoContext* c, const ProtoMap* m) {
        PairVec out;
        for (const ProtoMapIterator* it = m->getIterator(c); it && it->hasNext(c); it = it->advance(c))
            out.emplace_back(it->nextKey(c), it->nextValue(c));
        return out;
    }
}

TEST(MapIterator, EmptyYieldsNothing) {
    ProtoSpace space;
    ProtoContext* c = space.rootContext;
    EXPECT_TRUE(drain(c, c->newMap()).empty());
}

TEST(MapIterator, MatchesProcessElementsInBothForms) {
    ProtoSpace space;
    ProtoContext* c = space.rootContext;
