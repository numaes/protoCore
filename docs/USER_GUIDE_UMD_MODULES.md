...
3. **Use the resolution chain (optional)**  
   To have UMD use your provider for a given logical path, set the `ProtoSpace` resolution chain to include an entry like `"provider:your_alias"` (or `"provider:GUID"`). If you don't set the chain, the platform default (path entries only) is used.

4. **Load modules**  
   Call `space.getImportModule(ctx, logicalPath, "exports")`, where `ctx` is a `ProtoContext*` of the calling thread (for example `space.rootContext` on the main thread). The returned wrapper object has an attribute `"exports"` pointing to the module your provider returned (or to the cached module).
...
| Use in chain | `space.setResolutionChain(chain->asObject(ctx))` with a `ProtoList` entry `"provider:alias"` or `"provider:GUID"`. |
| Load module | `space.getImportModule(ctx, logicalPath, "exports")`; read the wrapper's `"exports"` attribute. |
